...
public class FilterExpVisitor extends ExpBaseVisitor<Object> {
    ...
    public Filter visitStrInFieldPredicate(ExpParser.StrInFieldPredicateContext ctx) {
        ...
        if (columnExpr instanceof String) {
            ...
            if (ctx.NOT() != null) {
                return Filter.notInStringCollections(colName, stringArray);
            } else {
                return Filter.inStringCollections(colName, stringArray);
            }
        } else {
            throw new CompileException("unsupported filter, ctx: " + ctx.getText());
        }
    ...
    public Filter visitOpFieldPredicate(ExpParser.OpFieldPredicateContext ctx) {
        ...
        if (columnExpr instanceof String) {
            String colName = columnExpr.toString();
            if (ctx.GT() != null && value instanceof Number) {
                return Filter.gt(colName, (Number) value);
            } else if (ctx.GEQ() != null) {
                return Filter.gte(colName, (Number) value);
            } else if (ctx.LT() != null) {
                return Filter.lt(colName, (Number) value);
            } else if (ctx.LEQ() != null) {
                return Filter.lte(colName, (Number) value);
            } else if (ctx.EQ() != null) {
